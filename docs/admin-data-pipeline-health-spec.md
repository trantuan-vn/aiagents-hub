...
> **Bổ sung, không thay thế:** FinOps hạ tầng → [`admin-cloudflare-usage-spec.md`](./admin-cloudflare-usage-spec.md); inbox lỗi Worker → [`admin-cloudflare-logs-spec.md`](./admin-cloudflare-logs-spec.md); an toàn quy mô triệu user → [`scale-safety-million-users-spec.md`](./scale-safety-million-users-spec.md) (**Phase A:** pending soft/hard caps + `pending_high` hot users + cleanup `order_items`/`order_discounts`); system config queue/d1tor2 → `/dashboard/system-config`  
