...
| **2. Ingest** | Get DB Info chỉ liệt kê bảng. Save RAG: introspect bảng, đọc `ADMIN.DBTOOLS$EXECUTION_HISTORY`, một lần LLM (mô tả cột + typical query), embed schema và sqlexample. Handle `llm`. Bỏ đường PDF/text | Form → Get DB Info → Loop → Save RAG ghi hai document / bảng. Test không gọi `get-db-info/execute` từ Save RAG |
| **3. Hỏi** | Get RAG gom theo bảng, trả đủ schema rồi SQL example cho Reasoning Agent | Câu hỏi tiếng Việt retrieve được bảng có mô tả VI và vài câu SQL mẫu |
