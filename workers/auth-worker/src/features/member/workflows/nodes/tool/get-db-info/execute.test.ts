...
describe('executeGetDbInfoPipeline', () => {
  ...
  it('introspects a named table and returns loop items', async () => {
    const pragma = {
      results: [
    ...
    const db = {
      prepare: vi.fn((sql: string) => ({
        bind: (..._args: unknown[]) => ({
          all: async () => ({ results: sql.includes('SELECT *') ? [{ id: '1', name: 'a' }] : [] }),
          first: async () => ({ cnt: 1 }),
        }),
        all: async () => {
  ...
  it('connects to OCI Oracle using user, password, and connectString from the previous node', async () => {
    ...
    const out = await executeGetDbInfoPipeline(ctx);
    expect(directMock.introspectOracleTableDirect).toHaveBeenCalledWith(
